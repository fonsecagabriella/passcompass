...
best_run = (
    max(runs, key=lambda r: float(r.data.metrics.get(args.metric, -1e9)))
    if args.higher_is_better
    else min(runs, key=lambda r: float(r.data.metrics.get(args.metric, 1e9)))
)
...
client.transition_model_version_stage(
    name=args.model_name, version=result.version, stage="Staging", archive_existing_versions=True
)
