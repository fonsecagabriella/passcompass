...
def pick_and_register_best(
    ...
    key = metric
    best_run = max(runs, key=lambda r: r.data.metrics.get(key, float("-inf"))) \
        if higher_is_better else \
        min(runs, key=lambda r: r.data.metrics.get(key, float("inf")))
...
def promote_best_model_flow(
    ...
    """
    ver = pick_and_register_best(
        experiment, metric, higher_is_better, model_name, mlflow_uri
    )
    if ver:
