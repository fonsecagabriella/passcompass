...
@task
def save_report(snapshot, out_dir: Path) -> Tuple[Optional[Path], Optional[Path]]:
    """Save Evidently snapshot to *out_dir*; return paths (may be None)."""

    out_dir.mkdir(parents=True, exist_ok=True)
    html_p = out_dir / "drift_report.html"
    json_p = out_dir / "drift_report.json"

    saved_html = saved_json = False
    ...
    try:
        ...
        print(f"⚠️  Failed to save HTML: {exc}")

    # JSON
    try:
        if hasattr(snapshot, "save_json"):
            snapshot.save_json(json_p)
            saved_json = True
        elif hasattr(snapshot, "as_dict"):
            json_p.write_text(json.dumps(snapshot.as_dict(), default=str, indent=2), encoding="utf-8")  # type: ignore[attr-defined]
            saved_json = True
        else:
            json_p.write_text(snapshot.json(), encoding="utf-8")  # type: ignore[attr-defined]
            saved_json = True
    except Exception as exc:  # noqa: BLE001
        print(f"⚠️  Failed to save JSON: {exc}")

    if saved_html or saved_json:
        print("✅ Drift report saved to", out_dir.resolve())
    else:
        print("❌ No artefacts saved – check Evidently version")

    return (html_p if saved_html else None, json_p if saved_json else None)
...
def monitor_flow(
    ...
    if os.getenv("ENVIRONMENT", "local").lower() == "gcs" and gcs_uri:
        if html_p and html_p.exists():
            upload_to_gcs(html_p, gcs_uri, f"drift/{today}/{html_p.name}")
        if json_p and json_p.exists():
            upload_to_gcs(json_p, gcs_uri, f"drift/{today}/{json_p.name}")
